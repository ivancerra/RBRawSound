...
.set    GPIO_FSEL2_ALT0,    0x100                       @ Funcion Alt0 para Pin Unidad 2 de GPIO 
.set    GPIO_GPFSEL1,       0x4                         @ Registro selector de GPIO para primera decena 
...
@ Set GPIO 12 To Alternate PWM Function 0
    ldr     r0, =PERIPHERAL_BASE + GPIO_BASE
    ldr     r1, =GPIO_FSEL2_ALT0
...
@ Configuramos PWM
    ldr     r0, =252                                                 @ 500Mhz / 45 = 11.11 -> 11.11 / 0.0441 = 252 -> Perdemos de 256 a 252 de rango :( para 256 deberia ser 256 * 0.0441 = 11.29
    bl      sndSetPWM                                                @ Configuramos PWM

@ Configuramos DMA Canal 1 
    mov     r0, #1                                                  @ Configuramos DMA
    bl      sndSetDMA   
 
Loop:
    b       Loop                                                    @ Hasta el Infinito y mas alla
...
DMAControlBlockAudio:                                               @ Estructura de Bloque de Control
  ...
  .word     DMAControlBlockAudio                                    @ DMA Direccion del proximo bloque de control, yo mismo y asi hago Loop
